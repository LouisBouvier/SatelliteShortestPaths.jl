...
function charger_images_labels(chemin_dossier_data)
    ...
    @showprogress for (i, classe) in enumerate(readdir(chemin_dossier_data))
        ...
        for image_file in readdir(classe_path)
            ...
            push!(images, permutedims(loaded_im, [3, 2, 1]))
            push!(labels, i)
        end
    end
...
images, labels = charger_images_labels("C:\\Users\\a051799\\data\\EuroSAT_RGB")
...
# ╔═╡ bc3c38cc-74be-4f0e-892d-fd2ff2fa2037
begin
	# Créer les ensembles d'entraînement et de test
	train_data = [(images[i], Flux.onehotbatch(labels[i], 1:length(labels))) for i in train_indices]
	test_data = [(images[i], Flux.onehotbatch(labels[i], 1:length(labels))) for i in test_indices]
end
...
# ╔═╡ 05aeaf40-b246-4e38-ba39-df506d2099d2
train_data
...
begin
	...
	for epoch in 1:epochs
	    Flux.train!(loss, Flux.params(model), train_data, opt, cb = Flux.throttle(() -> println("Epoch $epoch"), 2))
	end
end

# ╔═╡ 9a94a0e8-4d2c-4cac-be51-044b8e525006
begin
	# Évaluation du modèle
	function accuracy(data)
	    acc = 0
	    for (x, y) in data
	        acc += sum(onecold(model(x)) .== onecold(y)) / length(y)
	    end
	    return acc / length(data)
	end
	
	println("Précision sur les données d'entraînement : ", accuracy(train_data))
	println("Précision sur les données de test : ", accuracy(test_data))
end

# ╔═╡ 47417c0b-12da-449a-ac72-7ea12af4df4c
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "1a67cf81e367916eff99eb506902beb253f75470"
