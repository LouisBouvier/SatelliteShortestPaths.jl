...
# ╔═╡ 63c2f502-e0a9-11ee-10a2-c9d0b54ab67c
begin
	using Flux
	using Random
	using FileIO
	...
	using Random
	using Images
	using Flux: Conv, Dense, relu, softmax, crossentropy, onehotbatch
...
# ╔═╡ 07c02359-f54f-4aae-816d-370b9bf1265b
md"
## Lecture des images
"

# ╔═╡ 1fa06fcd-74ce-4a68-ad2d-aa4b87ee6fb9
# Chargement des images et des labels
function charger_images_labels(chemin_dossier_data)
    images = Array{Float32}(undef, 27000, 3,64,64)
    labels = []
	cpt = 1
	print(size(images))
    @showprogress for (i, classe) in enumerate(readdir(chemin_dossier_data))
        classe_path = joinpath(chemin_dossier_data, classe)
        for image_file in readdir(classe_path)
            image_path = joinpath(classe_path, image_file)
			loaded_im = Float32.(channelview(load(image_path)))
            images[cpt, :, :, :] = loaded_im
            push!(labels, i)
			cpt+=1
        end
    end
...
# ╔═╡ 1a99b73b-328e-4ba5-8f99-bc8d84b6d911
begin
	images, labels = charger_images_labels("EuroSAT_RGB")
	label_train_enc = Flux.onehotbatch(labels, 1:10)
end

# ╔═╡ 1c3ace55-5c0f-49f9-916c-aa034491f6d7
begin
	# Fraction d'images à utiliser pour l'entraînement
	train_ratio = 0.8
	
	# Générer un ensemble aléatoire d'index
	num_images = size(images)[1]
	rand_indices = Random.randperm(num_images)
	
	# Calculer le nombre d'images à utiliser pour l'entraînement
	num_train = Int(floor(train_ratio * num_images))
	
	# Diviser les index en ensembles d'entraînement et de test
	train_indices = rand_indices[1:num_train]
	test_indices = rand_indices[num_train+1:end]
end

# ╔═╡ db315179-34c0-4747-a7cd-0b1e95b5a4f6
# Modèle CNN
model = Chain(
        Conv((3, 3), 3=>16, relu),
        MaxPool((2,2)),
        ...
        Dense(32*14*14, 256, relu),
        Dense(256, 10),
        softmax
    )

# ╔═╡ dea3f175-5c79-4fbf-8f3a-b116a85a7543
begin
	imag_train = reshape(images, (64,64,3,27000))
	train_im = imag_train[:, :, :, train_indices]
	train_lab = label_train_enc[:, train_indices]
	
	test_im = imag_train[:, :, :, test_indices]
	test_lab = label_train_enc[:, test_indices]
end

# ╔═╡ bfb70fe3-ddf1-41d9-b336-d366278c049e
begin
	train_ds = Flux.DataLoader((train_im, train_lab), batchsize=64, shuffle=true)
	test_ds = Flux.DataLoader((test_im, test_lab), batchsize=64, shuffle=true)
end

# ╔═╡ c0b35303-0505-4958-8845-3533358ba879
begin
	nb_epochs = 10
	losses = []
	...
	@progress "Training epoch: " for epoch in 1:nb_epochs
		...
		for (d, l) in train_ds
			loss, grads = Flux.withgradient(model) do m
			            ...
			            y_hat = m(d)
			            Flux.crossentropy(y_hat, l)
			        end
			Flux.update!(optim, model, grads[1])
			push!(loss_tmp, loss)  # logging, outside gradient context
		end
	end
	push!(Statistics.mean(loss_tmp))
end

# ╔═╡ ca83229d-3a4f-4939-a567-771ce0ab9bd9
...
manifest_format = "2.0"
project_hash = "0b189c6f652f9e6d8b0f2146ce18cc6bd3eb4ddd"
